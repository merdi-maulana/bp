...
function Dome({
  ...
  // OKE: Me-load gambar 360 melalui URL CDN yang sudah diproses
...
export default function VirtualTour({
  ...
  const [sphereSegments, setSphereSegments] = useState<[number, number]>([
    48, 32,
  ]);
  ...
  return (
    ...
    >
      <style>{`
        ...
        /* Penajaman visual gambar HTML Image standar */
        .crisp-img {
      ...
      {/* Fullscreen Guide Overlay Modal */}
      {showGuide && (
        <div
          className={`absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity duration-500 pointer-events-none ${fadeGuide ? "opacity-0" : "opacity-100"}`}
        >
          <div className="max-w-md w-full px-6 flex flex-col items-center text-center text-white">
            <div className="flex justify-center gap-12 md:gap-16 mb-8 items-center">
              <div className="flex flex-col items-center gap-3">
                <div className="w-16 h-16 rounded-full border border-white/20 flex items-center justify-center bg-white/5 shadow-[0_0_15px_rgba(255,255,255,0.05)]">
                  <svg
                    className="w-8 h-8 animate-rotate-phone text-[#92F7C3]"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1.5"
                ...
                <span className="text-xs font-semibold max-w-[120px] leading-relaxed">
                  Putar HP Horizontal
                </span>
              </div>
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="w-16 h-16 rounded-full border border-white/20 flex items-center justify-center bg-white/5 shadow-[0_0_15px_rgba(255,255,255,0.05)]">
                  <svg
                    className="w-8 h-8 text-[#92F7C3] flex items-center justify-center"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1.5"
                  ...
                  >
                    ...
                    <path
                      d="m19 12-7-7-7 7"
                      className="animate-swipe-horizontal"
                      style={{
            ...
            <h2 className="text-xl md:text-2xl font-display font-bold mb-2">
              Petunjuk Tur Virtual 3D
            </h2>
            <p className="text-white/70 text-sm leading-relaxed max-w-xs">
              Gunakan mode landscape/horizontal pada handphone Anda dan geser
              layar untuk menjelajahi seluruh sudut ruangan.
            </p>
          </div>
        </div>
      )}
      ...
      {showGuide && (
        <div
          className={`absolute bottom-8 left-1/2 transform -translate-x-1/2 z-10 pointer-events-none transition-opacity duration-500 ${fadeGuide ? "opacity-0" : "opacity-100"}`}
        >
          <div className="bg-black/40 backdrop-blur-md px-6 py-3 rounded-full border border-white/20 shadow-xl">
            <p className="text-white/90 text-sm font-medium flex items-center gap-2">
              <svg
                ...
                strokeLinejoin="round"
              >
                <path d="M5 9c0-2.8 2.2-5 5-5s5 2.2 5 5-2.2 5-5 5H5z" />
                <path d="M5 9v10" />
                <path d="m9 16-4 3-4-3" />
              </svg>
      ...
      >
        ...
        >
          <group
            rotation={[0, scene.initialRotation || 0, 0]}
                ...
                <Html center>
                  <div className="flex flex-col items-center gap-3 text-center max-w-xs">
                    ...
                    <div className="text-white font-medium">
                      Gagal memuat panorama
                    </div>
                    <div className="text-white/60 text-sm">
                      Gambar tidak ditemukan atau gagal dimuat dari server.
                    </div>
                  </div>
                </Html>
              }
            >
              {/* PERUBAHAN UTAMA: Memasukkan path yang sudah dikonversi ke helper getCDNUrl */}
              <Dome
                imagePath={getCDNUrl(scene.imagePath)}
          ...
          <div className="info-modal bg-black/75 z-[120] text-white flex flex-col font-sans border-white/10 overflow-y-auto">
            ...
            {scene.info.imagePath && (
              <div className="relative w-full h-48 md:h-56 shrink-0 bg-zinc-950">
                {/*
                  PERUBAHAN UTAMA:
                  1. Menggunakan src dengan getCDNUrl()
                  2. Menambahkan unoptimized karena memuat resource eksternal CDN gratis tanpa Next-image Optimization Server
                  3. Menambahkan class crisp-img untuk menjaga ketajaman pixel
                */}
                <Image
                  src={getCDNUrl(scene.info.imagePath)}
                  alt={scene.title}
                  fill
                  unoptimized
                  sizes="(max-width: 768px) 100vw, 50vw"
                  className="object-cover crisp-img"
                />
            ...
            <div className="p-6 md:p-8 flex-1 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                  <span className="text-[10px] text-white/55 uppercase tracking-widest block mb-1 font-semibold">
                    Luas
                  </span>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                  ...
                  <span className="text-sm md:text-base font-bold text-emerald-400">
                    {scene.info.dimensions}
                  </span>
                </div>
              </div>

              <div className="flex-1">
                <span className="text-[10px] text-white/55 uppercase tracking-widest block mb-2 font-semibold">
                  Deskripsi
                </span>
